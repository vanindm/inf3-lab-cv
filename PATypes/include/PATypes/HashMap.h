...
namespace PATypes {
	...
	public:
		...
		std::shared_ptr<HashMap<K, V2>> map(std::function<V2(V)>& func) {
			std::shared_ptr<HashMap<K, V2>> newMap = std::make_shared<HashMap<K,V2>>(mod);
			std::shared_ptr<Sequence<V>> all = GetAll();
			auto allEnumerator = all->getEnumerator();
			while(allEnumerator->moveNext()) {
				newMap->Set(allEnumerator->current().key, func(allEnumerator->current().value));
			}
		}
		virtual void Add(K key, V value) {
			...
			if (current == nullptr) {
				storage.set(hash, std::make_shared<HashMapNode>(key, value, nullptr));
			} else {
		...
		std::shared_ptr<Sequence<V>> GetAll() {
			std::shared_ptr<MutableListSequence<V>> result = std::make_shared<MutableListSequence<V>>();
